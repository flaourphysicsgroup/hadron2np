...
def three_body_phase_space_factor(m_IS, m_1, m_2, m_3, m_12):
    factor_1 = 1.0 / ((2 * np.pi)**3 * m_IS)
    factor_2 = np.sqrt(kallen(m_IS**2, m_12**2, m_3**2)) / (8 * m_IS**2)
    factor_3 = np.sqrt(kallen(m_12**2, m_1**2, m_2**2)) / (8 * m_12**2)
    return factor_1 * factor_2 * factor_3

def four_body_phase_space_factor(m_IS, m_1, m_2, m_3, m_4, m_12, m_34, width_34, theta2):
    factor_1 = np.pi * np.sin(theta2) / ((2 * np.pi)**6 * m_IS * m_34 * width_34)
